...
MODEL_PATHS = [
    MODEL_DIR / "waste_mobilenet.keras",
    MODEL_DIR / "waste_mobilenet.h5",
]
...
def cargar_modelo():
    ...
    st.error(
        "No se encontró el modelo. Coloque la carpeta modelo_reciclaje_mobilenet junto a app.py."
    )
    st.stop()
...
if archivo:
    ...
    st.success(
        f"Predicción principal: **{resultados[0][0]}** ({resultados[0][1]:.2f}%)"
    )
